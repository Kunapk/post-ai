...
class _NumpadDialogState extends State<NumpadDialog> {
  ...
  void initState() {
    super.initState();
    // ✅ Initialize with total amount (so user doesn't have to type it)
    totalPayment = widget.totalAmount ?? 0;
    controller = TextEditingController(text: formatter.format(totalPayment));
    debugPrint(
      '💰 Numpad initialized: totalAmount=${widget.totalAmount}, totalPayment=$totalPayment',
    );
  }
  ...
  Widget build(BuildContext context) {
    return Column(
      children: [
        NumpadHeader(total: widget.totalAmount),
        const Divider(),
        buildNumpadCashType(),
              ...
              child: Padding(
                ...
                child: ElevatedButton(
                  onPressed: totalPayment >= widget.totalAmount!
                      ? () {
                          debugPrint('✅ [Numpad] Checkout button pressed');
                          debugPrint('   Total Amount: ${widget.totalAmount}');
                          debugPrint('   Payment: $totalPayment');
                          double changeAmount =
                              totalPayment - widget.totalAmount!;
                          debugPrint('   Change: $changeAmount');
                          Navigator.of(context).pop();
                          widget.onCheckout(changeAmount, totalPayment);
                          debugPrint('   ✅ onCheckout callback called');
                        }
                      : null,
  ...
  Widget buildNumpad() {
    return SizedBox(
      ...
      child: GridView.count(
        ...
        children: List.generate(numPadData.length, (index) {
          NumPad numPad = numPadData[index];
          return Card(
            child: InkWell(
              child: Center(child: numPad.title),
